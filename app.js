...
const state = { clientes: [], pessoas: [], riscos: [], criterios: [], orgView: "cliente", adminView: "pessoas" };
...
async function api(path, opts) {
  const res = await fetch(path, opts && { ...opts, headers: { "Content-Type": "application/json", ...(opts.headers || {}) } });
  if (!res.ok) {
    let detail;
...
async function loadPainel() {
  const [clientes, resumo, pessoas] = await Promise.all([
    api("/api/clientes"), api("/api/dashboard/resumo"), api("/api/pessoas"),
  ]);
...
function populateFiltros(clientes) {
  ...
  buSel.innerHTML = `<option value="">BU Director (todos)</option>` + diretores.map(d => `<option value="${d.id}">${esc(d.nome)}</option>`).join("");
  industrySel.innerHTML = `<option value="">Industry Code (todos)</option>` + industries.map(i => `<option value="${esc(i)}">${esc(i)}</option>`).join("");
...
function directorColor(index) { return DIRECTOR_COLORS[index % DIRECTOR_COLORS.length]; }
function dmsLabel(dms) { return dms && dms.length ? dms.map(d => d.nome).join(", ") : "—"; }
...
function renderPainelSecoes() {
  ...
  const linhas = state.clientes.filter(c => {
    ...
    if (busca) {
      const haystack = [c.nome, c.am && c.am.nome, ...(c.dms || []).map(d => d.nome)].filter(Boolean).join(" ").toLowerCase();
...
document.getElementById("ms-salvar").addEventListener("click", async () => {
  ...
  const payload = {
    cliente_id: clienteId, pilar, status: msSelectedStatus,
...
document.getElementById("nr-salvar").addEventListener("click", async () => {
  ...
  if (!titulo) { alert("Informe um titulo."); return; }
  const payload = {
...
function populaFiltroPilarRiscos() {
  ...
  sel.innerHTML = `<option value="">Pilar (todos)</option>` + PILAR_ORDEM.map(p => `<option value="${p}">${PILAR_LABELS[p]}</option>`).join("");
}
...
document.getElementById("ap-salvar").addEventListener("click", async () => {
  ...
  const payload = { nome, papel: document.getElementById("ap-papel").value, ativo: document.getElementById("ap-ativo").checked };
  try {
    if (id) { await api(`/api/pessoas/${id}`, { method: "PUT", body: JSON.stringify(payload) }); }
    else { await api("/api/pessoas", { method: "POST", body: JSON.stringify(payload) }); }
    resetFormPessoa();
    await loadAdminPessoas();
  } catch (e) { alert("Nao foi possivel salvar: " + e.message); }
});
...
document.getElementById("ac-salvar").addEventListener("click", async () => {
  ...
  const payload = {
    nome, industry_code: industry, tipo_linha: document.getElementById("ac-tipo-linha").value,
    ...
    dm_ids: dmIds, ativo: document.getElementById("ac-ativo").checked,
  };
  try {
...
function exportarExcel(linhas, colunas, nomeArquivo, nomeAba) {
  if (typeof window.XLSX === "undefined") { alert("Exportador ainda carregando, tente novamente em instantes."); return; }
...
document.getElementById("btn-export-painel-excel").addEventListener("click", () => {
  const linhas = state.clientes.map(c => ({
    cliente: c.nome, industry: c.industry_code, bu_director: c.bu_director ? c.bu_director.nome : "",
    am: c.am ? c.am.nome : "", dms: dmsLabel(c.dms), modificado: fmtData(c.modificado),
    prazo: c.pilares.prazo, escopo: c.pilares.escopo, rh: c.pilares.rh, contrato: c.pilares.contrato, faturamento: c.pilares.faturamento,
  }));
  exportarExcel(linhas, [
    { header: "Cliente", key: "cliente", width: 22 }, { header: "Industry Code", key: "industry", width: 14 },
    { header: "BU Director", key: "bu_director", width: 20 }, { header: "AM", key: "am", width: 20 },
    { header: "DM(s)", key: "dms", width: 28 }, { header: "Modificado", key: "modificado", width: 14 },
    { header: "Prazo", key: "prazo", width: 10 }, { header: "Escopo", key: "escopo", width: 10 },
    { header: "RH", key: "rh", width: 10 }, { header: "Contrato", key: "contrato", width: 10 }, { header: "Faturamento", key: "faturamento", width: 12 },
...
document.getElementById("btn-export-riscos-excel").addEventListener("click", () => {
  const linhas = state.riscos.map(r => ({
    cliente: r.cliente_nome, pilar: PILAR_LABELS[r.pilar] || r.pilar, tipo: r.tipo, titulo: r.titulo,
    severidade: r.severidade, responsavel: r.responsavel || "", data_alvo: r.data_alvo || "", status: r.status,
  }));
  exportarExcel(linhas, [
    { header: "Cliente", key: "cliente", width: 22 }, { header: "Pilar", key: "pilar", width: 14 },
    { header: "Tipo", key: "tipo", width: 12 }, { header: "Titulo", key: "titulo", width: 40 },
    { header: "Severidade", key: "severidade", width: 12 }, { header: "Responsavel", key: "responsavel", width: 22 },
    { header: "Data alvo", key: "data_alvo", width: 12 }, { header: "Status", key: "status", width: 12 },
    ...
    return `
      ...
      <table>
        ...
        <tbody>
          ${clientesDoDir.map(c => `
            <tr>
              <td>${esc(c.nome)}</td><td>${esc(c.industry_code)}</td><td>${c.am ? esc(c.am.nome) : "—"}</td>
              <td>${esc(dmsLabel(c.dms))}</td><td>${c.pilares.prazo}</td><td>${c.pilares.escopo}</td>
              <td>${c.pilares.rh}</td><td>${c.pilares.contrato}</td><td>${c.pilares.faturamento}</td>
            </tr>
          `).join("")}
        </tbody>
      </table>
...
init().catch(err => {
  ...
  document.querySelector(".shell").insertAdjacentHTML("afterbegin",
    `<div class="modal-error" style="margin:20px 0">Falha ao carregar a aplicacao: ${esc(err.message)}. Verifique se o backend esta rodando.</div>`);
});
